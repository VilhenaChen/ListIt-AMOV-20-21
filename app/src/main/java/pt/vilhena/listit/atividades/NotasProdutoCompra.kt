...
class NotasProdutoCompra : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_notas_produto_compra)

        dados=intent.getSerializableExtra("dados") as Dados
        posicaoProd = intent.getIntExtra("posicaoProduto",0)
        posicaoList = intent.getIntExtra("posicaoLista",0)

        editNotas.setText(dados.getArrayListas()[posicaoList].getListaProdutos()[posicaoProd].notas)
    }

    fun onClickBtnSave(view: View) {
        if(editNotas.text.toString() != dados.getArrayListas()[posicaoList].getListaProdutos()[posicaoProd].notas)
        {
            dados.getArrayListas()[posicaoList].getListaProdutos()[posicaoProd].notas = editNotas.text.toString()
        }
